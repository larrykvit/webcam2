	...
	else
	{
		...
		for( DWORD i = len / 3 - 1; i < len/3 ; i -- )
		{
					...
					abs( buff[ i*3 + 2 ] - refBuff[i*3 + 2] ) > colourThreshold )
			{
				...
				if( P_NO_MOTION == motionFrame[ pixelLeft( count ) ] && P_NO_MOTION == motionFrame[ pixelAbove( count ) ] )
				{
					maxTarget++;

					motionFrame[ count ] = maxTarget;

					targetList.push_back( Target( xOf( count ), yOf( count ) ) );
				}
				//case 2a: the pixel above is not a motion pixel, thus pixel to the left is a motion pixel
				else if( P_NO_MOTION == motionFrame[ pixelAbove( count ) ] )
				{
					motionFrame[ count ] = motionFrame[ pixelLeft( count ) ];
					targetList[ motionFrame[ count ] ].size++;

					//check if the new pixel is out of the bounds, and if so, set new bounds
					//for this case, only xmax needs to be checked

					if( xOf( count ) > targetList[ motionFrame[ count ] ].xmax )
						targetList[ motionFrame[ count ] ].xmax = xOf( count );
				}
				//case 2b: the pixel to the left is not a motion pixel, thus the pixel above is a motion pixel
				else if( P_NO_MOTION == motionFrame[ pixelLeft( count ) ] )
				{
					motionFrame[ count ] = motionFrame[ pixelAbove( count ) ];
					targetList[ motionFrame[ count ] ].size++;

					//check if the new pixel is out of the bounds, and if so, set new bounds
					//for this case, only ymax needs to be checked

					if( yOf( count ) > targetList[ motionFrame[ count ] ].ymax )
						targetList[ motionFrame[ count ] ].ymax = yOf( count );
				}
				//case 3: both pixels are motion pixels, and they need to be grouped
				else
				{
					//first check if they are the same
					if( motionFrame[ pixelAbove( count ) ] == motionFrame[ pixelLeft( count ) ] )
					{
						motionFrame[ count ] = motionFrame[ pixelLeft( count ) ];// pixel left choosen arbitrarily
						targetList[ motionFrame[ count ] ].size++;
					}
					else
					{
						int smalli = pixelAbove( count );
						int bigi = pixelLeft( count );

						if( motionFrame[ pixelAbove( count ) ] > motionFrame[ pixelLeft( count ) ] )
						{
							smalli = pixelLeft( count );
							bigi = pixelAbove( count );
						}
						//always choose the target with the smallest index
						motionFrame[ count ] = motionFrame[ smalli ];
						targetList[ motionFrame[ count ] ].size++;
						...
						else
						{
							...
							{
								//should implemet check if the lower index target's related target is not -1
